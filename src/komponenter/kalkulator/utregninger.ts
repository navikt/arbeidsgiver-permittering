...
const kuttAvDatoIntervallInnefor18mnd = (
...
export const settDatoerInnenforRiktigIntervall = (
    datoIntervall: DatoIntervall[],
    startDato: Date
): DatoIntervall[] => {
    ...
    datoIntervall.forEach((intervall, indeks) => {
        if (datoIntervallErDefinert(intervall)) {
            const datoInnenfor18mndsperioden = kuttAvDatoIntervallInnefor18mnd(
                intervall,
                startDato,
                sluttDato
            );
            if (
                datoIntervallErForandret(intervall, datoInnenfor18mndsperioden)
            ) {
                datoerEndret = true;
            }
...
export const konstruerTidslinje = (
    ...
    const sluttDato = finnSistePermitteringsdato(
        allePermitteringerOgFravær.permitteringer
    );
    if (
        startDato &&
        sluttDato &&
        allePermitteringerOgFravær.permitteringer.length
    ) {
        ...
        for (let dagteller = 0; dagteller <= antallDagerIPeriode; dagteller++) {
            ...
            const aktuellDatoMedKategori = finneKategori(
                aktuellDato,
        ...
        const sisteAktiveDag =
            listeMedTidslinjeObjekter[listeMedTidslinjeObjekter.length - 1]
                .dato;
        const gjenståendeDagerI18Mnd = antalldagerGått(
            sisteAktiveDag,
            dato18mnd
        );
        ...
        for (
            let dagteller = 0;
            dagteller < gjenståendeDagerI18Mnd;
            dagteller++
        ) {
